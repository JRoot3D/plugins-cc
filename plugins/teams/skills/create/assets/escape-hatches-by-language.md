...
Reference data used by `/teams:create` when filling the `{{ESCAPE_HATCH_LIST}}` placeholder in `agent-templates/flow-reviewer.md` and `agent-templates/flow-implementer.md`. Pick the section matching the project's `language` from `.flow-spec/project.md` and render its bullets verbatim into both templates.
